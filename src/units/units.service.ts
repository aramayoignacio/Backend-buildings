  ...
  async create(createUnitDto: CreateUnitDto) {
    try {
      const sector = await this.sectorRepository.findOne({
        where: { id: createUnitDto.sectorId },
      });
      const type = await this.unitTypeRepository.findOne({
        where: { id: createUnitDto.typeId },
      });
      const unit = this.repository.create({
        sector,
        type,
        keys: createUnitDto.keys,
  ...
  async findOne(id: number) {
    try {
      const unit = await this.repository.findOne({
        where: { id },
        relations: ['sector.floor.building', 'garages', 'type', 'owners', 'tenants'],
      });
      if (!unit) {
        throw new NotFoundException();
      }
      return responseOk(unit);
    } catch (err) {
      console.error(err);
      throw new InternalServerErrorException();
    }
